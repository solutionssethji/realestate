...
function AgentsContent() {
  ...
  const handleToggleStatus = async (agent: Agent) => {
    ...
    try {
      ...
      toast.success(`Agent access ${newStatus === 'DISABLED' ? 'disabled' : 'enabled'} successfully.`);
      ...
      render: (a: Agent) => (
        <div className="flex items-center gap-2">
          <button
            ...
            title={a.status === 'DISABLED' ? 'Enable Access' : 'Disable Access'}
          >
            {statusLoading === a.id ? (
    ...
    <div className="space-y-6 pb-8">
          ...
          <DataTable
            ...
            emptyState={
              <EmptyState
                ...
                title="No Agents"
                description="There are no sales agents matching your criteria."
              />
            }
          />
        )}
      ...
      >
        <p className="text-slate-600">
          Are you sure you want to permanently delete the agent <strong className="text-slate-900">{agentToDelete?.fullName}</strong>? This action cannot be undone.
        </p>
      </Modal>
    </div>
  );
