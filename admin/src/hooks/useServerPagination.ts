...
export function useServerPagination<T = any>({
  ...
  const fetchData = useCallback(async (pageIndex: number, resetHistory = false) => {
    try {
      setLoading(true);
      ...
      if (debouncedSearch && searchField) {
        currentFilters.push({ field: searchField, operator: ">=", value: debouncedSearch });
        currentFilters.push({ field: searchField, operator: "<=", value: debouncedSearch + '\uf8ff' });
      }
      ...
      const res = await api.get(endpoint, {
        ...
        sortField: debouncedSearch ? searchField : (currentFilters.length === 1 && currentFilters[0].operator === '==' ? currentFilters[0].field : (currentFilters.length > 0 ? undefined : defaultSortField)),
      ...
      toast.error(error?.message || `Failed to load data`, { duration: 6000 });
    } finally {
      setLoading(false);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [endpoint, itemsPerPage, searchField, debouncedSearch, defaultSortField, JSON.stringify(filters), pageHistory]);
