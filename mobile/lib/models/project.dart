...
class Project {
  ...
  factory Project.fromJson(Map<String, dynamic> json) {
    ...
    if (json['facilities'] != null) {
      parsedAmenities = (json['facilities'] as List)
          .map((f) => BilingualHelper.get(f))
          .toList();
    } else if (json['amenities'] != null) {
      parsedAmenities = (json['amenities'] as List)
          .map((a) => BilingualHelper.get(a))
          .toList();
    }

    final projectPhotos =
        (json['projectPhotos'] as List<dynamic>?)
            ?.map((e) => e.toString())
            .toList() ??
        [];
    ...
    return Project(
      ...
      description: BilingualHelper.get(
        json['description'],
      ), // Fallback to empty if not in DB
      coverImage: projectPhotos.isNotEmpty
          ? projectPhotos.first
          : (json['coverImage'] as String? ?? ''),
      gallery: projectPhotos.isNotEmpty
          ? projectPhotos
      ...
      plotCount:
          int.tryParse(json['availablePlots']?.toString() ?? '') ??
          (json['plots'] != null ? (json['plots'] as List).length : 0),
      googleMap: json['googleMap'] as String? ?? '',
      projectVideo: json['projectVideo'] as String? ?? '',
      isFeatured: json['isFeatured'] as bool? ?? false,
      siteLayout: json['siteLayout'] as String? ?? '',
